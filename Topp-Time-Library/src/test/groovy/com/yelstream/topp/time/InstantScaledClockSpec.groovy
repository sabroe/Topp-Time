...
class InstantScaledClockSpec extends Specification {
    ...
    void "Verify basic scaling at selected granularities"() {
        ...
        Instant initialTimestamp = Instant.now(clock)

        when:
        InstantScaledClock scaledClock = new InstantScaledClock(clock, scaleOperator, initialTimestamp)
        Instant scaledDateTime = scaledClock.getScaledTimestamp(initialTimestamp + durationToAdd)
